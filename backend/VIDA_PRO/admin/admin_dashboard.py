...
st.divider()
st.subheader("Paiement Wave reçu")

sites = [c["site"] for c in clients]

if sites:
    selected_site = st.selectbox("Choisir client", sites)
    montant_wave = st.number_input("Montant reçu (FCFA)", min_value=0, value=50000, step=5000)

    if st.button("Valider paiement Wave"):
        clients = load_clients()
        payments = load_payments()

        for c in clients:
            if c["site"] == selected_site:
                c["paid"] = True
                c["expires"] = str(datetime.now().date() + timedelta(days=30))
